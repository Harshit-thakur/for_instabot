#written by harshit chauhan.


import requests   #imported requests library

app_access_token="3618601120.986d0b2.b682489c9a994619ada9c8759a3afed3"   #acess token from insta
...
def post_a_new_comment(insta_username):
    my_comment = raw_input("enter the comment")
    media_id = get_users_recent_posts(insta_username)
...
def search_comments(insta_usename):
    word = raw_input("Enter word you want to search:")
    media_id = get_users_recent_posts(insta_username)
...
choice=raw_input("Enter the choice from above options")
print choice
if choice=="a":
    info()
elif choice=="b":
